...
单独安装或更新 Command Line Tools 不等于项目已切换编译器。当前支持完整 Xcode
bundle，不支持以 `/Library/Developer/CommandLineTools` 为活动开发目录；不要仅因
独立 CLT 更新而切换。实际 Xcode/SDK 改变后需要重新进行兼容验证，但无需因此
修改正式 CI 认证基线。适用安全公告、宿主不再支持旧工具或实际功能需求仍可能
要求升级；CLT 的版本号更新本身不是这类证据，也不证明完整 Xcode 已获得安全修复。
...
工具及祖先目录还必须通过 macOS 扩展 ACL 检查。无 ACL、deny-only 和只读/搜索
等不授予修改能力的 ACL 可保留；授予写入、添加/替换子项、删除、修改权限或所有者
的 allow ACE 一律拒绝（即使主体看似可信，也不自动推断组成员关系）。未知、过大
或读取失败的 ACL 同样拒绝。不会清除用户 ACL；若出现此诊断，先核实精确授权需求
和路径，不应关闭检查或批量改动系统权限来继续构建。
