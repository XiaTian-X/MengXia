...
Current verified completed slice: `TASK-001 DONE`; `TASK-002 DONE`. The accepted Option A sequencing makes TASK-004 active and keeps TASK-003 pending until durable Library owner/lock context exists. Specification v1.1.12 incorporates the accepted TASK-004 contract and the implemented SQLite/error/reopen plus macOS path/ACL/root/lock, intent, staging/publish/recovery, bounded lifecycle, complete corruption and same-OS SIGKILL slices; all fourteen local gates pass while formal reviewed CI attestation remains pending. ADR-0006 resolves the finite macOS FFI/build-evidence boundary, and the synchronized Plan record therefore keeps TASK-004 `IN_PROGRESS`. No TASK-003 or later capability is authorized. A later capability MUST remain disabled while its own BLOCKER/OQ or task-start gate is open; TASK-004 local PASS is neither final completion evidence nor authority for another task.
