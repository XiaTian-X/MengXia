...
The following are conservative, versioned foundation defaults. They are configurable only through typed validated configuration; missing, zero, overflow or out-of-range values fail startup or disable the dependent capability.

| Boundary | Default | Hard accepted range / behavior |
|---|---:|---|
| Core Protobuf frame | 4 MiB | 64 KiB–16 MiB; media bytes forbidden |
| Decode/validation nesting | 64 levels | deeper input rejected before persistence |
| DB writer queue | 256 commands | 16–4096; full queue returns `BACKPRESSURE` |
| DB read connections | 4 | 1–16 |
| DB busy wait budget | 5 seconds | bounded typed timeout; no infinite wait |
| Stream buffer per ingest | 8 MiB | 1–32 MiB |
| Storage I/O workers | 2 | 1–8 |
| Hash workers | 2 | 1–8 |
| Concurrent ingests | 2 | 1–8 |
| Single ingest byte limit | 1 TiB | validated before copy and during stream |
| Aggregate staging limit | 2 TiB logical ceiling | also constrained by actual free space |
| Filesystem free-space reserve | max(10 GiB, 5% of volume) | an operation is rejected before violating reserve |
...
- TASK-002..005 no longer need to invent unbounded or magic limits.
