...
| 尚无 IPC、CAS 或产品能力；TASK-004 的实现、deterministic corruption/crash matrices、14-ID gate script 和 formal-CI wiring 已闭环，本机 exact attested-class tuple 全量通过，但 reviewed CI attestation 尚未产生 | repository file inventory, scoped TASK-004 diff review and local gate output | `EXPECTED_GAP` | 提交后由 reviewed `macos-26` CI 产生 formal attestation；不得偷跑 TASK-003+ |
