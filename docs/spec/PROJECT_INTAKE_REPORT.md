...
| Dependabot security updates, secret scanning, push protection and private vulnerability reporting are enabled; CodeQL default setup is configured. GitHub's current API rejects explicit Rust selection but automatic selection launched Actions, C/C++ and Rust analysis | GitHub settings/default-setup API read-back; in-progress initial CodeQL run `34563494593` | `FACT / EXTERNAL API INCONSISTENCY / MAINT-001 IN PROGRESS` | claim scan coverage only after each observed job succeeds; retain Clippy, cargo-deny, dependency review and repository security tests, and require only observed check names after the first result |
