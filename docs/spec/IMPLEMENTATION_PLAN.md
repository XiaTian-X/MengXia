...
date: "2026-08-21"
language: "zh-CN"
source_of_truth: "IMPLEMENTATION_SPEC.md v1.1.2"
review: "IMPLEMENTATION_REVIEW.md v1.1.1"
---
...
| Review | historical findings retained; no open blocker applies to TASK-001..005 | same, with reproducible evidence | `FACT` |
...
Current plan state: `READY_FOR_TASK_001`. Implementation must still follow task dependency order; later gated capabilities remain disabled.

## 3. Phase 0 — intake, evidence and decision gate

Status: `DONE` for the foundation scope required by TASK-001..005.
...
3. Accepted incremental `OQ-006` caps: `DONE` for TASK-002..005 in ADR-0005; later caps close before their consuming tasks.
...
5. A doc traceability check proving every referenced `FUNC/REQ/DATA/API/SEC/REL/CFG/AC/TASK/OQ` ID exists. Current ID inventory/fence/whitespace check: `DONE`; TASK-001 must make it reproducible in CI.
6. `IMPLEMENTATION_REVIEW.md` updated so no BLOCKER applies to TASK-001..005: `DONE` for current evidence.
...
```text
Feature/Requirement/AC IDs; exact files and dependency direction;
...
|---|---|---|---|---|---|---|
| `TASK-001` Repository bootstrap | `PENDING / NEXT` | FUNC-001; SEC-020, DATA-006 | Phase 0 and OQ-003 accepted | root Cargo/toolchain/deny config, crate skeletons | build/metadata/dependency/advisory policy smoke | Pin fixed SQLite path; no historical names or relaxed dependency edges |
...
| `TASK-006` Asset domain/persistence | `PENDING` | FUNC-002, FUNC-003; REQ-001, REQ-002, REQ-004, REQ-005, REQ-008, REQ-011, REQ-012; DATA-009, DATA-010, DATA-011 | TASK-004, TASK-005 | domain/app/store, complete immutable `0001_library_assets` | AC-002, AC-005..008, FK/sequence/concurrency/event tests | No migration rewrite after apply; Blob dedup never merges Asset |
| `TASK-007` copy-only ingest slice | `PENDING` | FUNC-002; REQ-010, REQ-013; API-003, API-010; REL-005 | TASK-003, TASK-006 | app/proto/CLI/store/storage | AC-001..009, E2E, concurrent duplicate, all crash points | Copy only; reject adopt/reference; physical durability before registration |
...
| `TASK-012` exact OS sandbox | `BLOCKED` | FUNC-007; SEC-001, SEC-002, SEC-005, SEC-021 | TASK-011; OQ-001, OQ-002; resource caps | platform sandbox/host/security tests | AC-020..023 + mandatory real hostile suite | All required dimensions ENFORCED or deny; no backend-name/self-report shortcut |
...
| `TASK-022` retention/GC/Purge | `BLOCKED` | FUNC-011; REQ-015; DATA-010, DATA-011; SEC-018, SEC-019 | TASK-008, TASK-013, TASK-021; OQ-008 | admin/app/storage/store/CLI | AC-041..043; preview/hold/last-copy/concurrency/crash tests | Purge disabled until policy; exact target set; no retirement→delete inference |
...
|---|---|---|---|
| 0 Intake/decisions | documentation only | repository inspected | `DONE`; ADR-0003..0005; no blocker applies to TASK-001..005 |
