...
fn canonical_documents_have_closed_stable_id_traceability() {
    ...
    let task_008_proposal =
        fs::read_to_string(root.join("docs/proposals/TASK-008-GATE-PROPOSAL.md"))
            .expect("TASK-008 accepted contract is readable");
    let task_008_adr = fs::read_to_string(
        root.join("docs/spec/adr/ADR-0011-task-008-read-and-materialization-boundary.md"),
    )
    .expect("ADR-0011 is readable");
    validate_task_008_start_gate(
        &plan.text,
        &task_008_proposal,
        specification,
        &decisions.text,
...
task003_run TEST-IPC-MACOS-001 -- ./scripts/run-task-003-second-uid.sh";
    ...
    for stale_specification in [
        ...
        specification.replace(
            "TASK-011 caller-supplied private Plugin protocol/session 与 hostile fixture 已通过 PR `#12` reviewed run `34761111053` 和 merged-main run `34761648787`，`AC-101`、`AC-102`、`AC-103` 与十二项稳定测试全部通过，authority 已撤销为 `NONE`",
            "TASK-010 pure foundation 仍处于 `IN_PROGRESS`，authority 为 `TASK_010_FOUNDATION_ONLY`；TASK-011+",
        ),
        specification.replace(
            "TASK-010 与 TASK-011 当前均为 `DONE / NONE`",
...
fn validate_task_011_start_gate(
    ...
    for (name, text) in [
        ...
        for required in [
            ...
            if !text.contains(required) {
                return Err(format!(
                    "{name} lacks TASK-011 completion marker: {required}"
                ));
            }
        }
    }
    if proposal.contains("READINESS: READY_TO_IMPLEMENT_TASK_011") {
        return Err("TASK-011 proposal retains stale implementation readiness".to_owned());
    }
    ...
    for required in [
        "Status: DONE under accepted proposal v0.1.2, ADR-0017",
        "reviewed PR run `34761111053` and merged-main run `34761648787`",
    ...
    if !plan.contains("### TASK-011 start record — 2026-09-13")
        ...
        || !plan.contains("### TASK-011 completion record — 2026-09-13")
        || !review.contains("TASK-011 PRIVATE PROTOCOL DONE / WHOLE V1 NOT READY")
    ...
    {
        return Err("TASK-011 completion evidence is incomplete".to_owned());
    }
    let completion = plan
        .split("### TASK-011 completion record — 2026-09-13")
        .nth(1)
        .and_then(|tail| tail.split("\n## 6. Phases and gates").next())
        .ok_or_else(|| "TASK-011 completion record body is missing".to_owned())?;
    for required in [
        ...
        "REQUIRED_UNEXECUTED_TESTS: NONE",
        "Lifecycle: TASK-011 is `DONE`; implementation authority is",
    ] {
        if !completion.contains(required) {
            return Err(format!("TASK-011 completion record lacks: {required}"));
        }
    }
    for id in ["AC-101", "AC-102", "AC-103"].iter().chain(test_ids.iter()) {
        if !extract_ids(completion)
            .iter()
            .any(|observed| observed == id)
        {
            return Err(format!("TASK-011 completion record is missing {id}"));
        }
    }
    for (name, text) in [
        ("Specification", specification),
        ...
        for forbidden in [
            ...
            if text.contains(forbidden) {
                return Err(format!("{name} retains stale TASK-011 state: {forbidden}"));
            }
        }
    }
    Ok(())
...
fn validate_task_010_review_candidate(
    ...
    if !review.contains("TASK-011 PRIVATE PROTOCOL DONE / WHOLE V1 NOT READY")
    ...
    if !repository_status.contains("TASK-010 与 TASK-011 当前均为 `DONE / NONE`")
    ...
    if !agents.contains("TASK-010 foundation、TASK-011 complete")
...
#[allow(clippy::too_many_arguments)]
fn validate_task_008_start_gate(
    plan: &str,
    proposal: &str,
    specification: &str,
    decisions: &str,
    ...
    const ACCEPTANCE: &[&str] = &["AC-017", "AC-018", "AC-019"];
    const TESTS: &[&str] = &[
        "TEST-PROTO-008",
        "TEST-CLI-008",
        "TEST-CONFIG-008",
    ...
    let row = plan
        ...
        .find(|line| {
            line.starts_with(
                "| `TASK-008` verify/recovery + Asset read/materialize + Core observability |",
            )
        })
        .ok_or_else(|| "TASK-008 Plan row is missing".to_owned())?;
    if !row.contains("| `DONE` |")
        || !row.contains("TASK-003, TASK-004, TASK-005, TASK-006, TASK-007 DONE")
        || !row.contains("proposal §3 exact files only")
        || !row.contains("reviewed run `34188886713` PASS")
    {
        return Err("TASK-008 Plan row lacks its exact completed scope/evidence".to_owned());
    }
    if !proposal.contains("status: \"ACCEPTED_INCORPORATED_BY_CANONICAL_SPECIFICATION_1_1_31\"")
        || !proposal.contains("TASK008_PROPOSAL_VERSION: 0.2.5")
        || !proposal.contains("TASK008_UNRESOLVED_BLOCKING_FINDINGS: NONE")
    {
        return Err("TASK-008 proposal is not the completed v0.2.5 contract".to_owned());
    }
    if !adr.starts_with("# ADR-0011:")
        || !adr.contains("- Status: ACCEPTED")
        || !adr.contains("Normative detail: `docs/proposals/TASK-008-GATE-PROPOSAL.md` v0.2.5")
    {
        return Err("TASK-008 requires accepted ADR-0011 bound to proposal v0.2.5".to_owned());
    }
    let record = "TASK008_CANONICAL_GATE: ACCEPTED\n\
...
TASK008_IMPLEMENTATION_AUTHORITY: NONE";
    for (name, document) in [
        ...
        if document.match_indices(record).count() != 1 {
            return Err(format!(
                "TASK-008 completion gate lacks one synchronized authority record in {name}"
            ));
        }
    }
    let completion = plan
        .split("### TASK-008 completion record")
        .nth(1)
        .and_then(|tail| tail.split("\n### Post-TASK-007 correction").next())
        .ok_or_else(|| "TASK-008 completion record body is missing".to_owned())?;
    for required in [
        "7aeb032a75edbe85050cf470d910bc53a85d74cf",
        "34188886713",
        "required unexecuted tests: `NONE`",
        ...
        if !completion.contains(required) || !proposal.contains(required) {
            return Err(format!(
                "TASK-008 completion evidence is missing from Plan/proposal: {required}"
            ));
        }
    }
    for id in ACCEPTANCE.iter().chain(TESTS) {
        if !extract_ids(completion)
            ...
            .any(|observed| observed == id)
        {
            return Err(format!("TASK-008 completion record is missing {id}"));
        }
    }
    let start_record = plan
        .split("### TASK-008 start record")
        .nth(1)
        .and_then(|tail| tail.split("\n### TASK-008 completion record").next())
        .ok_or_else(|| "TASK-008 start record body is missing".to_owned())?;
    for id in ACCEPTANCE.iter().chain(TESTS).chain(DECISIONS) {
        if !definitions.contains_key(*id) {
            return Err(format!(
                "TASK-008 stable ID lacks canonical definition: {id}"
            ));
        }
        if !extract_ids(start_record)
            .iter()
            .any(|observed| observed == id)
        {
            return Err(format!("TASK-008 start record is missing {id}"));
        }
        if !extract_ids(proposal).iter().any(|observed| observed == id) {
            return Err(format!("TASK-008 proposal is missing {id}"));
        }
    }
    for required in [
        "DEVELOPER_GATE: scripts/verify-task-008.sh developer",
        "FORMAL_COMPLETION_GATE: scripts/verify-task-008.sh formal",
        "migrations/root rebind/deletion/Admin/TASK-009+ remain unauthorized",
        ...
        "crates/mengxia-testkit/tests/fixtures/task_007/handshake-v1.1.pb",
    ] {
        if !start_record.contains(required) && !proposal.contains(required) {
