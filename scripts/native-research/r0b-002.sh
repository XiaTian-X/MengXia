...
cd "$repository_root"
umask 077
research_root=$(/usr/bin/mktemp -d /tmp/mengxia-r0b002.XXXXXXXX)
printf 'R0B002_DIRECTORY=%s\n' "$research_root"
active_pid=''
cancel() {
    trap '' INT TERM HUP
    if [ -n "$active_pid" ]; then kill -TERM "$active_pid" 2>/dev/null || true; wait "$active_pid" 2>/dev/null || true; fi
    printf 'R0B002_CANCELLED directory=%s\n' "$research_root" >&2
    exit 130
}
...
for file in r0b-002.c r0b-002.sh controller.c; do
...
/usr/bin/env -i LANG=C LC_ALL=C TZ=UTC "$clang" -std=c11 -O0 -g -Wall -Wextra -Werror \
    -isysroot "$sdk" "$research_root/source/r0b-002.c" -o "$research_root/probe" > "$research_root/compile.stdout" 2> "$research_root/compile.stderr" &
active_pid=$!
...
printf 'R0B002_CAPTURE_EXIT=%s directory=%s\n' "$result" "$research_root"
